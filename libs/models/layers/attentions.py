...
class ROIGather(nn.Module):
    ...
    def forward(self, roi_features, fmap_pyramid, layer_index,time_token=None):
        """
        ...
        if self.cross_attention_weight > 0:
            context = self.attention(roi, fmap,time_token)
...
class AnchorVecFeatureMapAttention(nn.Module):
    ...
    def forward(self, roi, fmap,time_token=None):
        """
        ...
        """
        time_token = time_token.unsqueeze(2)
        
        query = self.f_query(roi)  # [B, Np, C] ->  [B, Np, C]
        t_query = self.t_query(time_token)
        t_query = t_query.reshape(query.shape[0],64)
        t_query = t_query.unsqueeze(1).repeat(1, 192, 1)
        
        query = query+t_query

        key = self.f_key(fmap)  # [B, C, H, W]
        key = self.resize(key)  # [B, C, H'W']
        t_key = self.t_key(time_token)
        t_key = t_key.reshape(query.shape[0],64)
        t_key = t_key.unsqueeze(2).repeat(1, 1, 250)
        ...
        t_value = self.t_value(time_token)
        t_value = t_value.reshape(query.shape[0],64)
        t_value = t_value.unsqueeze(2).repeat(1, 1, 250)
